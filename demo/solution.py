...
class AlgSolution:
    ...
    P6_ALIGN_Y_STEPS = 75       # ~3 s
    P6_ALIGN_Y_FWD = 0.0
    P6_ALIGN_Y_LAT = -0.5        # negative = right

    # Phase 7: push box FORWARD into the pit (arm extended)
    P7_PUSH_FWD_STEPS = 520      # ~5 s
    P7_PUSH_FWD_FWD = 0.4
    P7_PUSH_FWD_LAT = 0.0

    # Phase 7.5: pause — let box settle in the pit before crossing
    P75_PAUSE_STEPS = 100        # ~2 s, zero velocity, arm retracted
    ...
    def _apply_schedule(self):
        ...
        if s < p1:
            ...
            # Pause — let box settle, zero velocity, arm retracted
            fwd, lat = 0.0, 0.0
            self._arm_target = self.arm_default_action
            self.heading_kp = 2.0
            phase = "p75_pause"
